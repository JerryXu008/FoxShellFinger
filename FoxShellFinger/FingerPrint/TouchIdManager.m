...
static NSString *CURRENT_TOUCH_ID_IDENTITY = @"SUININGTOUCHTEST";
...
+ (BOOL)touchIdInfoDidChange
{
    ...
    if (!data && [self isErrorTouchIDLockout]) {
        //输入次数过多被锁定，此时指纹并没有变更
        NSLog(@">>>>>>>>>>>>touchIdInfoDidChange=1");
        return NO;
    }
    if (!data) {//当前没有指纹，清空s数据
        [TouchIdManager setCurrentTouchIdDataIdentity:@"SUININGTOUCHTEST"];
        [TouchIdManager setCurrentIdentityTouchIdData:nil];
        return NO;
    }
    NSData *oldData = [self currentIdentityTouchIdData];
    if (oldData == nil&&data==nil) {
        //应用内该账号未设置过指纹
        NSLog(@">>>>>>>>>>>>touchIdInfoDidChange=2");
        return NO;
    }
    else if (oldData == nil&&data!=nil) {
         //以前没有指纹，新增了指纹
        //保存指纹信息
        [TouchIdManager setCurrentTouchIdDataIdentity:@"SUININGTOUCHTEST"];
        ...
        NSLog(@">>>>>>>>>>>>touchIdInfoDidChange=3");
        //没有变化
        return NO;
    }
    else
    {
        NSLog(@">>>>>>>>>>>>touchIdInfoDidChange=4");
        //指纹信息发生变化
        return YES;
    }
}
...
+ (BOOL)setCurrentIdentityTouchIdData:(NSData *)data
{
    ...
    else if(data==nil){//情况数据
      
        [SAMKeychain deletePasswordForService:TOUCH_ID_DATA_SERVICE account:[self accountForKeychainWithIdentify]];
